...
#------- all merged data without TBP2 KO and P7
...
count.matrix.sel <- count.matrix[, -c(14, 15, 17)]

samples <- colnames(count.matrix.sel)
...
dds <- DESeqDataSetFromMatrix(countData = count.matrix.sel,
                              colData = info.df,
                              design = ~ condition)
...
pdf(file = "Figures/PCA_merged_rep_mESC_PGCs_oocyte_no_P7_2-cell_embryo.pdf", height = 8, width = 6)
print(p)
dev.off()